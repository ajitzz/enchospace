...
import { Shield, Trash2, Edit, Plus, Users, Home, CreditCard, TrendingUp, Calendar, MapPin, DollarSign, Activity, Search as SearchIcon, Phone as PhoneIcon } from 'lucide-react';
                ...
                >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-50/50">
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider">Customer</th>
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider">Property</th>
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider">Dates</th>
                      ...
                      <tbody>
                        {filteredBookings.map((booking: any) => (
                          <tr key={booking.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="py-4 px-6 text-sm text-gray-600">{booking.property_title}</td>
                            <td className="py-4 px-6 text-sm text-gray-600">
                              {new Date(booking.start_date).toLocaleDateString()} - {new Date(booking.end_date).toLocaleDateString()}
                            </td>
                            <td className="py-4 px-6 font-bold text-gray-900">${booking.total_price}</td>
                            <td className="py-4 px-6">
                              <select 
                                value={booking.status}
                                onChange={(e) => updateBookingStatus(booking.id, e.target.value)}
                                className={`text-xs font-bold px-2.5 py-1 rounded-full border-none outline-none cursor-pointer ${
                ...
                >
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-gray-50/50">
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider">User</th>
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider">Phone</th>
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider">Joined</th>
                          <th className="py-4 px-6 font-bold text-gray-500 text-xs uppercase tracking-wider text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                            <td className="py-4 px-6">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="font-bold text-gray-900">{u.user_name}</div>
                              </div>
                            </td>
                            <td className="py-4 px-6 text-sm text-gray-600">{u.user_phone}</td>
                            <td className="py-4 px-6 text-sm text-gray-600">{new Date(u.created_at).toLocaleDateString()}</td>
                            <td className="py-4 px-6 text-right">
                              <button 
                                onClick={() => window.open(`tel:${u.user_phone.replace(/\D/g, '')}`, '_self')}
                                className="text-sm font-bold text-blue-600 hover:text-blue-700 flex items-center justify-end gap-1 ml-auto"
                              >
